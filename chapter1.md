...
对所需功能的妥协使人们不断尝试提高现有流处理引擎（如，开发Storm Trident是为了克服Storm的许多限制）。当现有平台存在短板时，应用开发人员就需要担负起处理这些短板引起的问题。举个例子，在微批处理并没有提供事件中自然发生的会话与处理引擎中窗口之间良好的适配，而窗口仅仅是由多个批时间（恢复间隔）构成的。由于缺乏灵活性和表达能力，开发时间变慢，算子需要花费更所的精力来正确维护。
...
2014年Stratosphere代码的一个分支捐献给Apache软件基金会作为一个孵化项目，该系统的核心开发人员构成了其初始的commiter。在这之后很短的时间，许多初始的commiter离开大学并创办了一家名为data Art公司来商业化Flink。在孵化阶段，由于与一个不相干的项目存在混淆，这个项目不得不改变原来Stratosphere的名字。Flink被选中来代表这个流和批处理系统：在德语中，flink有快速或敏捷之意。图标选择了一个多彩的松鼠，因为松鼠行动快速而敏捷--宛如柏林的松鼠--拥有迷人的红棕色的外表，正如你在图 1-3所见的那样。

*左：柏林的大耳朵红松鼠。右：Apache Flink的大尾巴松鼠图标。它的颜色代表Apache基金会的图标。它是Apache风格的松鼠。*

该项目很快完成了孵化阶段，在2014年12月，Flink最终成为Apache基金会顶级项目。Flink是Apache基金会五大大数据项目之一，在全球范围内拥有超过200个开发人员，包括全球500强在内的多个商业公司使用Flink。到写作本书时，全球多个城市开办了34场Apche Flink的交流会，大约有12000名参会人员和Flink演讲者。在2015年10月，Flink项目在柏林举办了第一次年会：Flink Forward。

## 批处理和流处理

Flink为何以及为什么要同时进行批处理和流处理？Flink将批处理--即，处理静态、有限数据集--看做是流处理的一个特例。

Flink的计算架构核心，图1-4中被标记为“Flink runtime”的部分，是一个接收数据流程序的分布式系统，该系统在一个或多个机器上以容错方式运行这些程序。该runtime可以运行在集群中，或作为YARN（Yet Another Resource Negotiator）的一个应用运行，很快也可以运行在Mesos上（正在开发中），或者单机运行，这对Flink应用进行调试很有用。

*图1-4 这幅图描绘了Flink技术栈的主要部分。注意：用户层包括流处理和批处理的APIs，使Flink成为能够同时处理这两种情况的简单工具。软件库主要包括机器学习（FlinkML），复杂时间处理（CEP），和图计算（Gelly），以及流式处理和批处理的Table API。*

runtime接收的程序非常强大，但同时非常冗长和难以直接编写。由于这个原因，Flink在runtime上提供非常友好的开发接口，可以生成数据流代码。Flink有进行流式处理的DataStream API和进行批处理的DataSet API。很有趣的是，虽然Flink的runtime是基于数据流的，DataSet API却早于DataStream API出现，这是因为在Flink最初几年，工业界对处理无限数据流的需求还不普遍。

DataStream API在分析无限数据流时是一套非常有效的API。这些API可以是Java的、也可以是Scala的。用户使用一个被称为DataStream的数据结构用来表示分布式无限数据流。

Flink可以运行在具有成百上千台机器构成的分布式集群上，将一个很大的计算任务分成多个小任务，每台机器仅执行其中一个小任务。当某个节点出现故障或其他出现其他错误，以及在bug修复或版本升级人为重新处理时，Flink框架会自动正确恢复计算。这个功能缓解了编程人员对处理失败的担忧。Flink内部使用具有容错能力的数据流，允许开发人员分析不断产生的无限数据流（流式处理）。
